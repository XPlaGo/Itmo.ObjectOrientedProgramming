...
namespace BankAccountService.Application.Features.Transfers.Commands.Transfer.DepositToCardTransfer;

public class DepositToCardTransferCommandHandler : IRequestHandler<TransferCommand, Result<TransferResponse>>
{
    private readonly IUnitOfWorkWithRepositoriesFactory _unitOfWorkWithRepositoriesFactory;
    private readonly ICurrencyConversionService _conversionService;

    public DepositToCardTransferCommandHandler(
    ...
    public async Task<Result<TransferResponse>> Handle(TransferCommand request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            DepositAccount fromAccount = await unitOfWork
            ...
            await unitOfWork.DepositAccountRepository.UpdateAsync(fromAccount).ConfigureAwait(false);
            await unitOfWork.CardAccountRepository.UpdateAsync(toAccount).ConfigureAwait(false);
