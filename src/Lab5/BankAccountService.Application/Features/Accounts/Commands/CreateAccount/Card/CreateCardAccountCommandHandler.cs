...
namespace BankAccountService.Application.Features.Accounts.Commands.CreateAccount.Card;

public class CreateCardAccountCommandHandler : IRequestHandler<CreateAccountCommand, Result<CreateAccountResponse>>
{
    private readonly IUnitOfWorkWithRepositoriesFactory _unitOfWorkWithRepositoriesFactory;
    private readonly ICurrencyConversionService _currencyConversionService;

    public CreateCardAccountCommandHandler(IUnitOfWorkWithRepositoriesFactory unitOfWorkWithRepositoriesFactory, ICurrencyConversionService currencyConversionService)
    {
    ...
    public async Task<Result<CreateAccountResponse>> Handle(CreateAccountCommand request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            long id = await unitOfWork.CardAccountRepository.AddAsync(cardAccount).ConfigureAwait(false);
