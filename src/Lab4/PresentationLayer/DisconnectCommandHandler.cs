...
{
    public DisconnectCommandHandler(ICommandHandler? successor)
    {
        Successor = successor;
    }
    ...
    public string HandleCommand(string command, ICommandReceiver<string> receiver)
    {
        ...
        if (command.StartsWith("disconnect ", StringComparison.Ordinal))
        {
            var connectCommand = new DisconnectCommand(receiver);
