...
public class FileCopyCommandHandler : ICommandHandler
{
    public FileCopyCommandHandler(ICommandHandler? successor)
    {
        Successor = successor;
    }
    ...
    public string HandleCommand(string command, ICommandReceiver<string> receiver)
    {
        ...
        if (command.StartsWith("copy ", StringComparison.Ordinal))
        {
            ...
            var copyCommand =
                new FileCopyCommand<string>(receiver, new Path(args[1]), new Path(args[2]));
